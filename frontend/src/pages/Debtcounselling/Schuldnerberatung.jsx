...
export default function Schuldnerberatung() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#8B806D] via-[#515565] to-[#2B3041] text-slate-100 flex flex-col lg:flex-row font-sans overflow-x-hidden">
      <Toaster position="top-right" />

      {/* SIDEBAR */}
      <div className="w-full lg:w-[30%] xl:w-1/3 p-8 lg:p-10 xl:p-20 flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-white/10 bg-[#262B3D] lg:min-h-screen lg:sticky lg:top-0">
        <div>
          <h1 className="text-3xl lg:text-3xl xl:text-5xl font-serif italic mb-4 text-white">
            {t("title")}
          </h1>
          <p className="text-white text-base lg:text-lg xl:text-2xl">
            {t("subtitle")}
          </p>
        </div>
        <div className="mt-8 space-y-6 lg:mt-8 xl:space-y-10">
          <Feature
      ...
      {/* MAIN FORM AREA */}
      <div className="flex-1 flex flex-col justify-start lg:justify-center p-6 lg:p-12 xl:p-24 relative overflow-y-auto min-h-screen lg:min-h-0">
        <button
          onClick={() => navigate(-1)}
          className="fixed top-6 right-6 lg:absolute lg:top-8 lg:right-8 xl:top-12 xl:right-12 text-white hover:opacity-70 transition z-50"
        >
          <X size={28} />
        </button>
        ...
        <div className="max-w-xl w-full mx-auto mt-10 lg:mt-0">
          {/* Progress */}
          <div className="mb-8 lg:mb-10 xl:mb-16">
            <div className="text-xl lg:text-2xl uppercase tracking-widest text-[#BA8C61] mb-4 font-bold">
              {step + 1} / {totalSteps}
            </div>
            <div className="h-[2px] bg-white/10">
              <motion.div
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              exit={{ opacity: 0 }}
            >
              <h2 className="text-2xl lg:text-3xl xl:text-4xl font-serif italic mb-6 lg:mb-8 xl:mb-10 text-white">
                {titles[step]}
              </h2>
                ...
                {step === 4 && (
                  <div className="space-y-3 lg:space-y-4">
                          ...
                          {["account", "salary", "assignment", "unknown"].map(
                            (type) => (
                              <button
                                ...
                                className={`p-3 text-base lg:text-base xl:text-lg text-left border transition-all ${
                                  formData.garnishType === type
                                    ? "border-[#BA8C61] bg-[#BA8C61] text-white"
                                    : formData.garnished
                                      ? "border-white/10 bg-white/5 text-white/90 hover:border-white/30"
                    ...
                    >
                      <div className="mt-4 pt-4 border-t border-white/5">
                        <p
                          className={`text-base lg:text-base xl:text-lg tracking-widest mb-3 transition-colors ${formData.vehicle ? "text-white" : "text-white/50"}`}
                        >
                          {t("extendedStatements.vehicleDocs")}
                        </p>
                        <div className="flex gap-2">
                          {["self", "bank"].map((loc) => (
                            <button
                              ...
                              className={`flex-1 p-3 text-base lg:text-base xl:text-lg border transition-all ${
                                formData.vehicleDocLocation === loc
                                  ? "border-[#BA8C61] bg-[#BA8C61] text-white"
                                  : formData.vehicle
                                    ? "border-white/10 bg-white/5 text-white/90 hover:border-white/30"
                ...
                {step === 6 && (
                  <>
                    <textarea
                      ...
                      value={formData.notes}
                      onChange={handleChange}
                      className="w-full bg-white/5 border border-white/10 p-4 lg:p-6 outline-none h-36 lg:h-40 text-white text-lg xl:text-xl focus:border-[#BA8C61] transition-all"
                    />
                    <Checkbox
                      name="privacy"
                      label={t("fields.privacy")}
              ...
              {/* Nav Buttons */}
              <div className="flex justify-between mt-10 lg:mt-12 xl:mt-16 pb-8 lg:pb-10 xl:pb-12">
                <button
                  onClick={back}
                  className={`${step === 0 ? "invisible" : ""} flex items-center gap-2 text-white text-lg xl:text-xl hover:opacity-50 transition`}
                >
                  <ArrowLeft size={24} /> {t("buttons.back")}
                </button>
                <button
                  onClick={step === 6 ? submitForm : next}
                  className="bg-white text-[#262B3E] px-8 lg:px-10 py-3 lg:py-4 font-bold hover:bg-[#BA8C61] hover:text-white flex items-center gap-2 transition-all text-lg xl:text-xl"
                >
                  {step === 6 ? t("buttons.submit") : t("buttons.next")}
                  <ChevronRight size={18} />
                </button>
...
const StaticGroupCard = ({ name, label, checked, onChange, children }) => {
  return (
    <div
      className={`transition-all duration-500 border rounded-sm p-4 lg:p-5 ${
        checked
          ? "bg-white/10 border-[#BA8C61]/50 shadow-lg"
          : "bg-white/5 border-white/5"
      }`}
    >
      <label className="flex items-center gap-4 cursor-pointer group">
        ...
        <span
          className={`text-lg xl:text-xl transition-all ${checked ? "text-white font-medium" : "text-white"}`}
        >
          {label}
        </span>
      </label>
...
const Input = ({ label, error, ...props }) => (
  <div className="w-full">
    <label className="text-[13px] xl:text-[17px] uppercase tracking-[0.2em] text-white block mb-2">
      {label}
    </label>
    <input
      {...props}
      className={`w-full bg-transparent border-b py-2 lg:py-3 outline-none text-lg xl:text-xl transition-colors ${error ? "border-red-400" : "border-white/20 focus:border-[#BA8C61]"}`}
    />
    {error && <p className="text-red-400 text-base lg:text-lg mt-2">{error}</p>}
  </div>
);
  ...
  <div>
    <label className="flex items-center gap-4 cursor-pointer group">
      <input
        ...
        checked={checked}
        onChange={onChange}
        className="accent-[#BA8C61] w-5 h-5 lg:w-6 lg:h-6"
      />
      <span className="text-lg xl:text-xl text-white group-hover:text-[#BA8C61] transition-colors">
        {label}
      </span>
    </label>
    {error && <p className="text-red-400 text-base lg:text-lg mt-2">{error}</p>}
...
const Select = ({ name, label, value, onChange, options, error }) => (
  <div>
    <label className="text-[13px] xl:text-[17px] uppercase tracking-[0.2em] text-white block mb-3">
      {label}
    </label>
    <div className="grid grid-cols-1 gap-2 lg:gap-3 max-h-[220px] lg:max-h-[260px] xl:max-h-[300px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-[#BA8C61]">
      {options.map((opt) => (
        <button
          key={opt.val}
          type="button"
          onClick={() => onChange({ target: { name, value: opt.val } })}
          className={`p-3 lg:p-4 border text-left text-lg xl:text-xl transition-all ${value === opt.val ? "border-[#BA8C61] bg-[#BA8C61]/10 text-white" : "border-white/10 text-white/70 hover:border-white/30"}`}
        >
          {opt.lab}
        </button>
      ))}
...
const Feature = ({ icon, title, desc }) => (
  <div className="flex items-start gap-4 lg:gap-5">
    <div className="text-[#BA8C61] mt-1 shrink-0">{icon}</div>
    <div>
      <h4 className="font-semibold text-[13px] xl:text-[17px] uppercase tracking-[0.2em] text-white">
        {title}
      </h4>
      <p className="text-white text-[13px] xl:text-[17px] mt-2 leading-relaxed opacity-90">
        {desc}
      </p>
    </div>
  </div>
